 ...
 */
struct ReadBlobs : public Message {
  ...
  struct BlobAddress {
    ...
    uint32_t ifCommitIdHigher; // Only return the blob contents if the commit id is higher than specified here
  };
   ...
   */
  static MessagePointer Create(uint8_t databaseId, uint8_t nBlobsRequested = 1, bool writeLock = false);


  static constexpr Type type = Type::ReadBlobs;
private:
  ReadBlobs(uint8_t databaseId, uint8_t nBlobsRequested, bool writeLock); // Do not use the constructor -> use EncodeMessage
};
