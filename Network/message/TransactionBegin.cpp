...
TransactionBegin::TransactionBegin(message_size messageSize) : Message(messageSize, TransactionBegin::type) {}

MessagePointer_T<TransactionBegin> TransactionBegin::Create(int nDatabases) {
  ...
  return MessagePointer_T<TransactionBegin>(new (new char[messageSize]) TransactionBegin(messageSize));
}
